...
//			+ "copy_file(?, ?, cat.category_image, ? || (case when upper(lob.sub_lob_id) = 'COMMERCIAL' then 'commercial/1' else (case when upper(lob.sub_lob_id) = 'RESIDENTIAL' then 'residential/1' end) end), cat.category_image), "
			+ "copy_file(?, ?, cat.category_image, ? || (case when upper(lob.sub_lob_id) = 'COMMERCIAL' then 'commercial/1' when upper(lob.sub_lob_id) = 'RESIDENTIAL' then 'residential/1' when upper(lob.sub_lob_id) = 'COLLECTION' then 'collection/1' when upper(lob.sub_lob_id) = 'TILE-ART' then 'tile_art/1' end), cat.category_image), "			
